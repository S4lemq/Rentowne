...
import static pl.rentowne.tenant_settlement.service.payment.p24.RequestUtil.*;
...
public class PaymentMethodP24 {
    ...
    public String initPayment(TenantSettlement tenantSettlement) {
        ...
        ResponseEntity<TransactionRegisterResponse> result = p24Client.post().uri("/transaction/register")
                .bodyValue(createRegisterRequest(config, tenantSettlement))
                .retrieve()
    ...
    public String receiveNotification(TenantSettlement tenantSettlement, NotificationReceiveDto receiveDto, String remoteAddr) {
        ...
        validate(receiveDto, tenantSettlement, config);
    ...
    private String verifyPayment(NotificationReceiveDto receiveDto, TenantSettlement tenantSettlement) {
        ResponseEntity<TransactionVerifyResponse> result = p24Client.put().uri("/transaction/verify")
                .bodyValue(createVerifyRequest(config, tenantSettlement, receiveDto))
                .retrieve()
